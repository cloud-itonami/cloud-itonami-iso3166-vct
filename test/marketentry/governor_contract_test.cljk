...
(deftest clean-intake-auto-commits
  (let [[db actor] (fresh)
    ...
    (is (= "Bequia Marine Supplies Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest active-debarment-committee-is-held-and-unoverridable
  (testing "an active, unresolved Debarment Committee case -> HARD hold (flagship check, interim leg, s.64(3))"
    (let [[db actor] (fresh)
      ...
      (is (some #{:debarment-disqualifying} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest debarment-invalid-years-is-held
  (testing "a :debarment-sanction-years outside Schedule 4 §7(1)(c)'s {1,3,5} enum -> HARD hold, regardless of the apparent window"
    (let [[db actor] (fresh)
          _ (assess! actor "t5bpre" "eng-7")
          _ (draft! actor "t5bpre" "eng-7")
          res (exec-op actor "t5b" {:op :filing/submit :subject "eng-7"} operator)]
      (is (= :hold (get-in res [:state :disposition])))
      (is (some #{:debarment-disqualifying} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest debarment-expired-before-submission-is-clean
  (testing "a lawful temporary debarment whose computed window has already lapsed before submission-date -> NOT disqualifying, submit proceeds to the usual actuation escalation"
    (let [[db actor] (fresh)
          _ (assess! actor "t5cpre" "eng-6")
          _ (draft! actor "t5cpre" "eng-6")
          res (exec-op actor "t5c" {:op :filing/submit :subject "eng-6"} operator)]
      (is (= :interrupted (:status res)) "pauses for human approval (actuation), not a governor hold")
      (let [r2 (approve! actor "t5c")]
        (is (= :commit (get-in r2 [:state :disposition])))
        (is (true? (:submitted? (store/engagement db "eng-6"))))))))

(deftest permanent-debarment-is-held-with-no-expiry
  (testing "Schedule 4 §7(1)(d) Permanent Debarment -> ALWAYS a HARD hold, no date needed"
    (let [[db actor] (fresh)
          _ (assess! actor "t5dpre" "eng-8")
          _ (draft! actor "t5dpre" "eng-8")
          res (exec-op actor "t5d" {:op :filing/submit :subject "eng-8"} operator)]
      (is (= :hold (get-in res [:state :disposition])))
      (is (some #{:debarment-disqualifying} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest breached-conditional-non-debarment-converts-and-is-held
  (testing "Schedule 4 §7(1)(b): a breached Conditional Non-Debarment converts into an active Temporary Debarment -> HARD hold"
    (let [[db actor] (fresh)
          _ (assess! actor "t5epre" "eng-9")
          _ (draft! actor "t5epre" "eng-9")
          res (exec-op actor "t5e" {:op :filing/submit :subject "eng-9"} operator)]
      (is (= :hold (get-in res [:state :disposition])))
      (is (some #{:debarment-disqualifying} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest unbreached-conditional-non-debarment-is-clean
  (testing "an unbreached Conditional Non-Debarment never converts -> not disqualifying, escalates as usual"
    (let [[db actor] (fresh)
          _ (assess! actor "t5fpre" "eng-10")
          _ (draft! actor "t5fpre" "eng-10")
          res (exec-op actor "t5f" {:op :filing/submit :subject "eng-10"} operator)]
      (is (= :interrupted (:status res)))
      (let [r2 (approve! actor "t5f")]
        (is (= :commit (get-in r2 [:state :disposition])))
        (is (true? (:submitted? (store/engagement db "eng-10"))))))))

(deftest reprimand-is-clean
  (testing "Schedule 4 §7(1)(a) Reprimand 'falls short of debarment' -> never disqualifying"
    (let [[db actor] (fresh)
          _ (assess! actor "t5gpre" "eng-11")
          _ (draft! actor "t5gpre" "eng-11")
          res (exec-op actor "t5g" {:op :filing/submit :subject "eng-11"} operator)]
      (is (= :interrupted (:status res)))
      (let [r2 (approve! actor "t5g")]
        (is (= :commit (get-in r2 [:state :disposition])))
        (is (true? (:submitted? (store/engagement db "eng-11"))))))))
...
(deftest tin-unverified-is-held-and-unoverridable
  (testing "unverified TIN (Comptroller of Inland Revenue) when required -> HARD hold"
    (let [[db actor] (fresh)
