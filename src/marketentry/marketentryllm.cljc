(ns marketentry.marketentryllm
  ...
  the Saint Vincent and the Grenadines (VCT) public-sector market-entry
  compliance actor.
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world Central Procurement Board
  portal submission."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "debarment-committee-established?=" (:debarment-committee-established? e)
                        " debarment-sanction-kind=" (:debarment-sanction-kind e)
